# -*- coding: utf-8 -*-
"""
Created on Wed Apr  1 16:02:21 2026
...
#!/usr/bin/env python3
"""
Experimento 8: Dependencia de la degeneración con la dimensión del embedding.
Modelo: proyección radial (d_latent=2 fijo). Se reconstruye en diferentes dimensiones
de embedding (2, 3, 5, 10) mediante MDS. Se mide D (dimensión efectiva del espacio de soluciones),
I (correlación radial) y U = I·log(D). Se investiga cómo varían con la dimensión del embedding.
"""
...
# =============================================================================
# 2. RECUPERACIÓN MÚLTIPLE VÍA MDS CON DIMENSIÓN VARIABLE
# =============================================================================
...
def recover_multiple_embeddings(A_sparse, d_embed=2, n_solutions=20, verbose=False):
    """Recupera múltiples embeddings en dimensión d_embed."""
...
# =============================================================================
# 3. MÉTRICAS DE DEGENERACIÓN (ADAPTADAS A EMBEDDING DE CUALQUIER DIMENSIÓN)
# =============================================================================
...
def effective_dimension_of_solutions(solutions):
    ...
    Independiente de la dimensión del embedding.
    """
    n_sol = len(solutions)
    if n_sol < 3:
        return 1.0
...
# =============================================================================
# 4. EXPERIMENTO PARA UNA COMBINACIÓN (α, embedding_dim)
# =============================================================================

def run_experiment_embedding_dim(alpha, d_embed, n_seeds=5, n_solutions=20, N=500, verbose=False):
    """Ejecuta experimento para un α y una dimensión de embedding d_embed."""
    print(f"  α={alpha:.1f}, d_embed={d_embed}")
    all_solutions = []
    true_hs = []
    for seed in range(n_seeds):
        A_sparse, _, h_true, _ = generate_graph_radial_projection(seed, alpha, N=N, d_latent=2)
        true_hs.append(h_true)
        sols, _ = recover_multiple_embeddings(A_sparse, d_embed=d_embed,
                                              n_solutions=n_solutions, verbose=verbose)
...
def run_all_combinations(alphas, embed_dims, n_seeds, n_solutions, N, verbose):
    """Ejecuta experimentos para todas las combinaciones α × embed_dim."""
    results = {}
    for d_embed in embed_dims:
        print(f"\n--- Embedding dimension = {d_embed} ---")
        results[d_embed] = []
        for alpha in alphas:
            metrics = run_experiment_embedding_dim(alpha, d_embed, n_seeds, n_solutions, N, verbose)
            if metrics:
                results[d_embed].append(metrics)
    return results


def plot_results(results, output_dir="experiment_8"):
    """Genera gráficos comparativos para diferentes dimensiones de embedding."""
    embed_dims = list(results.keys())
    if not embed_dims:
        return

    plt.figure(figsize=(16, 12))

    # Subplot 1: Dimensión efectiva vs α
    plt.subplot(2,2,1)
    for d_embed in embed_dims:
        metrics = results[d_embed]
        alphas = [m['alpha'] for m in metrics]
        D = [m['effective_dimension_mean'] for m in metrics]
        plt.plot(alphas, D, 'o-', label=f'd_embed={d_embed}')
    plt.xlabel('α')
    ...
    # Subplot 2: Correlación radial vs α
    plt.subplot(2,2,2)
    for d_embed in embed_dims:
        metrics = results[d_embed]
        alphas = [m['alpha'] for m in metrics]
        I = [m['radial_correlation_mean'] for m in metrics]
        plt.plot(alphas, I, 'o-', label=f'd_embed={d_embed}')
    plt.xlabel('α')
    ...
    # Subplot 3: U = I·log(D) vs α
    plt.subplot(2,2,3)
    for d_embed in embed_dims:
        metrics = results[d_embed]
        alphas = [m['alpha'] for m in metrics]
        U = [m['U_mean'] for m in metrics]
        plt.plot(alphas, U, 'o-', label=f'd_embed={d_embed}')
    plt.xlabel('α')
    ...
    # Subplot 4: U vs d_embed para α fijo (por ejemplo α=2.0)
    alpha_fixed = 2.0
    d_embed_vals = []
    U_vals = []
    for d_embed in embed_dims:
        ...
        for m in metrics:
            if abs(m['alpha'] - alpha_fixed) < 0.1:
                d_embed_vals.append(d_embed)
                U_vals.append(m['U_mean'])
                break
    if d_embed_vals:
        plt.subplot(2,2,4)
        plt.plot(d_embed_vals, U_vals, 'o-', color='red')
        plt.xlabel('Dimensión de embedding')
        plt.ylabel(f'U en α={alpha_fixed}')
        plt.title('Dependencia de U con la dimensión de embedding')
        plt.grid(True)

    plt.tight_layout()
    plt.savefig(f"{output_dir}/comparison_embed_dim.png", dpi=150)
    plt.close()
    print(f"  Guardada figura: {output_dir}/comparison_embed_dim.png")


def main():
    parser = argparse.ArgumentParser(
        description="Experimento 8: Dependencia de la degeneración con la dimensión del embedding"
    )
    ...
    parser.add_argument("--embed_dims", type=str, default="2,3,5,10", help="Dimensiones de embedding a probar")
    parser.add_argument("--output_dir", type=str, default="experiment_8")
    ...
    print("="*70)
    print("Experimento 8: Dependencia de la degeneración con la dimensión del embedding")
    print(f"Modelo: proyección radial (d_latent=2 fijo)")
    ...
    results = run_all_combinations(alphas, embed_dims, args.n_seeds,
                                   args.n_solutions, args.N, args.verbose)
    elapsed = time.time() - start_time

    # Guardar resultados en JSON y TXT
    for d_embed, metrics in results.items():
        with open(f"{args.output_dir}/metrics_dembed_{d_embed}.json", 'w') as f:
            def convert(obj):
        ...
        with open(f"{args.output_dir}/metrics_dembed_{d_embed}.txt", 'w') as f:
